...
class MainFrame(wx.Frame):
    def __init__(self, poser: Poser,
        ...
        self.reader = imageio.get_reader(in_video)
        self.frame_idx = 0
        self.device = device
    ...
    def init_control_panel(self):
        # frame_size = self.reader.get_data(0).shape[:2][::-1]
        self.control_panel = wx.Panel(self, style=wx.SIMPLE_BORDER, size=(340, 256))
        self.control_panel_sizer = wx.BoxSizer(wx.VERTICAL)
        self.source_video_panel = wx.Panel(self.control_panel, size=(340, 256), style=wx.SIMPLE_BORDER)
    ...
    def load_default_image(self):
        image_file_name = "data/illust/source_image.png"
        pil_image = resize_PIL_image(extract_PIL_image_from_filelike(image_file_name))
        w, h = pil_image.size
        if pil_image.mode != 'RGBA':
            self.source_image_string = "Image must have alpha channel!"
    ...
    def update_result_image_panel(self, event: wx.Event):
        ...
        if self.frame_idx == self.reader.count_frames():
            self.frame_idx = 0
            # return
        rgb_frame = self.reader.get_data(self.frame_idx)
        # rgb_frame = cv2.resize(rgb_frame, (384, 512))
        ...
        pil_image = resize_PIL_image(cv2pil(bgr_frame), (340, 256))
        # pil_image = cv2pil(bgr_frame)
...
if __name__ == "__main__":
    ...
    parser.add_argument("in_video", help="input video")
    ...
    app = wx.App()
    main_frame = MainFrame(poser, face_detector, landmark_locator, args.in_video, cuda)
    main_frame.Show(True)
    main_frame.timer.Start(100)
    app.MainLoop()
